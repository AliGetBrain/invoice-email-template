...
const subject = `
Invoice #{{#each invoiceData}}{{invoiceNumber}}{{#unless @last}},{{/unless}}{{/each}}- Client: {{contactCompanyName}}, PastDue
`;
...
<head>
    ...
    <title>Weekly Invoice Reminder</title>
</head>
...
<body
    ...
    <div
        ...
        <p style="margin: 0 0 16px 0; font-size: 1rem; line-height: 1.6">
            We are reaching back out to you for an update on the following invoices as they have exceeded net terms and
            the resolution of these charges are required.
        </p>
...
function generateAndSaveHTML() {
  try {
    ...
    const outputFile = path.join(__dirname, "weekly_overdue.html");
