...
const subject = `
Invoice #{{#each invoiceData}}{{invoiceNumber}}{{#unless @last}},{{/unless}}{{/each}}- Client: {{contactCompanyName}}, Payment Not Found
`;
...
function generateAndSaveHTML() {
  try {
    ...
    const outputFile = path.join(__dirname, "payment_check.html");
